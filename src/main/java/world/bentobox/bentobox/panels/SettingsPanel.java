...
public class SettingsPanel {
    ...
    public static void openPanel(BentoBox plugin, User user, Flag.Type flagType, World world, int page) {
        ...
        if (page > 0) {
            // Previous page icon
            panelBuilder.item(new PanelItemBuilder().icon(Material.OAK_SIGN).name(user.getTranslation(PROTECTION_PANEL + "previous")).clickHandler((panel, user1, clickType, slot1) -> {
        ...
        if ((page + 1) * 44 < flags.size()) {
            // Next page icon
            panelBuilder.item(new PanelItemBuilder().icon(Material.OAK_SIGN).name(user.getTranslation(PROTECTION_PANEL + "next")).clickHandler((panel, user1, clickType, slot1) -> {
                openPanel(BentoBox.getInstance(), user, flagType, world, page + 1);
                return true;
            }).build());
        }
